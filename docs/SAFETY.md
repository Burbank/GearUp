...
Client (v2.0) holds ATIS 90s, TAF 90s, board 60s so tab switches do not multiply those budgets. Refresh and pull-to-refresh pass `{ force: true }` and `fresh=1`, which skip those holds and the in-process overheard / NAS / board caches. Tab return still uses the holds.
