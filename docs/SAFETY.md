...
`lib/liveatc.js`: hostname must be `liveatc.net` or `*.liveatc.net`. Service worker **must not** intercept audio (`destination === "audio"` return). Listen otherwise snaps back on HTTPS PWAs.
...
- Board: **10 / 60s / IP** (`BOARD_MAX`), separate bucket.
...
Client (v1.1) also holds ATIS 90s, TAF 90s, board 60s so tab switches do not multiply those budgets. Refresh buttons pass `{ force: true }`.
...
Netlify publishes `.` so **force 404** in `netlify.toml` for `/server.js`, `/package.json`, `/README.md`, `/docs/*`, `/lib/*`, `/netlify/*`, `/scripts/*`. Keep those redirects if you add more logs.
...
img-src 'self'
connect-src 'self'
media-src 'self' https:          # LiveATC streams
