...
const FAA_EXTRA_IATA = { PANC: "ANC", PHNL: "HNL", TJSJ: "SJU" };
...
async function getNasXml(fresh) {
  if (!fresh && xmlCache.until > Date.now() && xmlCache.xml) return xmlCache.xml;
...
function delayArgs(iataOrOpts) {
  if (iataOrOpts && typeof iataOrOpts === "object" && !Array.isArray(iataOrOpts)) {
    return {
      iata: String(iataOrOpts.iata || ""),
      fresh: !!iataOrOpts.fresh,
    };
  }
  return { iata: String(iataOrOpts || ""), fresh: false };
}

async function getDelay(rawIcao, iataOrOpts) {
  const code = String(rawIcao || "").toUpperCase();
  if (!isFaaIcao(code)) return { applicable: false, items: [] };
  const { iata, fresh } = delayArgs(iataOrOpts);
  const ident =
    iata.toUpperCase() ||
    FAA_EXTRA_IATA[code] ||
    (code[0] === "K" ? code.slice(1) : "");
  try {
    const xml = await getNasXml(fresh);
...
module.exports = { getDelay, isFaaIcao };
