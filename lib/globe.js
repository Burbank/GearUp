...
function globeRelFromEvent(event) {
  ...
  if (q.asset) return safeRelPath(q.asset);
  const rawUrl = String((event && event.rawUrl) || "");
  if (rawUrl) {
    try {
      const path = new URL(rawUrl).pathname;
      if (path === "/globe" || path === "/globe/") return "";
      if (path.startsWith("/globe/")) return safeRelPath(path.slice("/globe/".length));
    } catch {
      /* fall through */
    }
  }
  ...
  for (let i = 0; i < markers.length; i++) {
    ...
    if (at >= 0) return safeRelPath(path.slice(at + markers[i].length));
  }
  return "";
}
