...
async function fetchJsonList(url, ttlMs, opts) {
  const fresh = !!(opts && opts.fresh);
  const hit = listCache.get(url);
  if (!fresh && hit && hit.until > Date.now()) return hit.data;
  let data = [];
  try {
...
async function getNearbySigmets(icao, lat, lon, now, opts) {
  const listOpts = opts && opts.fresh ? { fresh: true } : undefined;
...
async function getNearbyAirmets(lat, lon, now, opts) {
  const rows = await fetchJsonList(
    "https://aviationweather.gov/api/data/gairmet?format=json",
    undefined,
    opts && opts.fresh ? { fresh: true } : undefined
  );
  const items = [];
  const seen = new Set();
...
async function getBriefWx(rawIcao, opts) {
  ...
  const settled = await Promise.allSettled([
    getDelay(icao, { iata: airport.iata, fresh }),
    getNearbySigmets(icao, lat, lon, now, { fresh }),
    faa ? getNearbyAirmets(lat, lon, now, { fresh }) : Promise.resolve([]),
    getPireps(icao),
    getMetar(icao),
  ]);
