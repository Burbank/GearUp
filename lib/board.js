...
function peekBoard(dir, nowMs, opts) {
  ...
  return {
    filling,
    fresh,
    skipLimit: filling || fresh || fills.size > 0,
  };
}
...
async function getBoard(dir, nowMs, opts) {
  ...
  if (hit && !hit.payload.partial && now - hit.at < CACHE_MS && !fill) {
  ...
  if (payloadHasRows(hit) && !hit.payload.partial) {
    return {
      ...hit.payload,
      revalidating: true,
      partial: false,
  ...
  if (related && related.payload && payloadHasRows(related)) {
    return {
      ...related.payload,
      aheadHours,
      partial: true,
