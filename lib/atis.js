...
function depSide(data) {
  if (!data) return null;
  if (data.departureAtis) return data.departureAtis;
  if (data.kind === "departure" || data.kind === "combined") return data;
  return null;
}
...
async function fetchAcars(icao) {
  const afP = fetchAirframes(icao).catch(() => null);
  const guruP = fetchGuru(icao).catch(() => null);
...
async function getAtis(rawIcao, opts) {
  ...
  if (opts && opts.quiet) {
    return fetchQuietAcars(icao);
  }

  if (icao === "VHHH") {
    try {
      const cad = await fetchVhhhCad();
      if (cad && cad.kind !== "empty" && cad.text) return asBundle(cad, icao);
    } catch {
  ...
  if (isNavCanada(icao)) {
    try {
      ...
      if (nc && nc.text) return asBundle(nc, icao);
    } catch (err) {
  ...
  if (isFaaDatis(icao)) {
    try {
      ...
      if (faa && (hasText(faa) || hasText(faa.arrivalAtis))) return asBundle(faa, icao);
    } catch {
      /* fall through to ACARS */
    }
  }

  return fetchAcars(icao);
}
