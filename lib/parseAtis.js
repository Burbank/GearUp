...
function cadCopy(html, cls, kind) {
  ...
  return {
    ...
    issueDayKnown: calendarDayKnown(best),
    text: best,
...
function looksCompleteAtis(text) {
  ...
  if (/ACKNOWLEDGE INFO/i.test(body) && body.length < 280) return false;
  if (
...
function pickAcars(...candidates) {
  return mergeAcars(...candidates);
}
