...
const NATO_WORD = Object.keys(NATO_LETTER)
  .sort((a, b) => b.length - a.length)
  .join("|");
const NATO_RE = new RegExp(`\\b(${NATO_WORD})\\b`, "i");
...
function extractLetter(text) {
  ...
  const received = text.match(
    new RegExp(
      `\\b(?:YOU HAVE )?RECEIVED ATIS\\s+(${NATO_WORD}|[A-Z])\\b`,
      "i"
    )
  );
  if (received) {
    const tok = received[1].toUpperCase();
    return NATO_LETTER[tok] || (tok.length === 1 ? tok : "");
  }
  const named = text.match(new RegExp(`\\bATIS\\s+(${NATO_WORD})\\b`, "i"));
  if (named) return NATO_LETTER[named[1].toUpperCase()] || "";
  const line = String(text)
    .split(/\n/)
    .map((s) => s.trim())
    .find((s) => NATO_RE.test(s) && s.replace(/\s+/g, "").length <= 10);
  if (line) {
    const m = line.match(NATO_RE);
    if (m) return NATO_LETTER[m[1].toUpperCase()] || "";
  }
  return "";
}
...
function issuedFromAtisBody(text, referenceIso, nowMs, notAfterIso) {
  ...
  const colon = head.match(/\b(\d{2}):(\d{2})Z\b/);
...
function looksCombinedAtis(text, extra) {
  const t = String(text || "");
  const hasArrHead = /\bARR(?:IVAL)?\s+(?:ATIS|INFO)\b/i.test(t);
  const hasDepHead = /\bDEP(?:ARTURE)?\s+(?:ATIS|INFO)\b/i.test(t);
  if (hasArrHead && hasDepHead) return true;
  if (/\bCOMBINED\s+(?:ATIS|INFO)\b/i.test(t)) return true;
  if (hasArrHead && !hasDepHead) return false;
  const icaoDepHead = /\b[A-Z]{4}\s+DEP(?:ARTURE)?\b/i.test(t);
  if ((hasDepHead || icaoDepHead) && !hasArrHead) return false;
  if (
    /\b(?:LDG|LANDING)\s+RWY\b/i.test(t) &&
    /\bDEP(?:ARTURE)?\s+RWY\b/i.test(t)
  ) {
    return true;
  }
  const land = extra && String(extra.landingRwy || "").trim();
  const dep = extra && String(extra.departureRwy || "").trim();
  return !!(land && dep);
}

function canadianAtisText(icao, letter, text) {
  const body = String(text || "").trim();
  if (!body) return "";
  if (
    /^\s*[A-Z]{4}\s+(?:DEP(?:ARTURE)?|ARR(?:IVAL)?|COMBINED)\s+ATIS\b/i.test(body)
  ) {
    return body;
  }
  const zulu = (body.match(/\b(\d{4})Z\b/) || [])[1] || "";
  const ident = letter || extractLetter(body);
  const kind = looksCombinedAtis(body) ? "combined" : parseA9Kind(body.slice(0, 80), "");
  const tag =
    kind === "arrival" ? "ARR ATIS" : kind === "departure" ? "DEP ATIS" : "ATIS";
  const head = [icao, tag, ident, zulu ? `${zulu}Z` : ""].filter(Boolean).join(" ");
  return `${head}\n${body}`;
}
...
function parseNavCanadaAtis(body, icao) {
  ...
  const kind = looksCombinedAtis(rawText, payload)
    ? "combined"
    : parseA9Kind(rawText.slice(0, 80), rawText.slice(0, 120));
  const text = canadianAtisText(code, letter, rawText);
...
function czechUpdated(html) {
  const raw = decodeEntities(String(html || ""));
  const m = raw.match(
    /Updated:\s*(\d{2})\.(\d{2})\.(\d{4})\s+(\d{2}):(\d{2})(?::(\d{2}))?\s*UTC/i
  );
  if (!m) return { issued: null, issuedText: "" };
...
function czechBody(raw) {
  const s = String(raw || "");
  const pre = s.match(/<pre\b[^>]*>([\s\S]*?)<\/pre>/i);
  if (pre) return cleanAtisText(pre[1]);
  if (/<(?:html|head|body|pre|table)\b/i.test(s)) return "";
  return cleanAtisText(s);
}

function parseCzechIbs(html, icao) {
  const code = String(icao || "").toUpperCase();
  const text = czechBody(html);
  const names = {
    LKPR: { iata: "PRG", name: "Prague" },
    LKTB: { iata: "BRQ", name: "Brno" },
    LKMT: { iata: "OSR", name: "Ostrava" },
    LKKV: { iata: "KLV", name: "Karlovy Vary" },
  };
  const meta = names[code] || { iata: "", name: "" };
  if (!text || !looksCompleteAtis(text)) {
    return {
      icao: code,
      iata: meta.iata,
      name: meta.name,
  ...
  const header = czechUpdated(html);
  const atHhmm = text.match(/\bAT\s+(\d{4})\b/);
  const fromAt = atHhmm
    ? zuluOnToday(atHhmm[1], header.issued)
    : { issued: null, issuedText: "" };
  const fromBody = issuedFromAtisBody(text, header.issued);
  const times = header.issued
    ? header
    : fromAt.issued
      ? fromAt
      : fromBody;
  const rec = {
    ...
    issued: times.issued,
    issuedText: times.issuedText,
    issueDayKnown: !!times.issued,
    text,
    source: "meteo.rlp.cz",
  };
  return withSides(code, rec, null, meta);
}
...
function parseA9Kind(header, bodyHead) {
  const joined = `${header}\n${bodyHead}`;
  if (looksCombinedAtis(joined)) return "combined";
  const h = joined.slice(0, 200);
  const hasArrHead = /\bARR(?:IVAL)?\s+(?:ATIS|INFO)\b/i.test(h);
  const hasDepHead = /\bDEP(?:ARTURE)?\s+(?:ATIS|INFO)\b/i.test(h);
  if (hasArrHead && !hasDepHead) return "arrival";
  if (hasDepHead && !hasArrHead) return "departure";
  if (/\bCOMBINED\s+(?:ATIS|INFO)?\b/i.test(h)) return "combined";
  if (/\bARRIVAL\b/i.test(h) && !/\bDEPARTURE\b/i.test(h)) return "arrival";
  if (/\bDEPARTURE\b/i.test(h) && !/\bARRIVAL\b/i.test(h)) return "departure";
  if (/\bARRIVAL\b/i.test(h) && /\bDEPARTURE\b/i.test(h)) return "combined";
  return "combined";
}
...
function parseA9(raw, icao, headerRe, receivedAt) {
  ...
  const kind = looksCombinedAtis(body)
    ? "combined"
    : parseA9Kind(headerMatch ? headerMatch[0] : body.slice(0, 80), body.slice(0, 120));
  const letter =
...
function sidesFrom(item) {
  ...
  if (!arrival && usableAtis(core) && (core.kind === "arrival" || core.kind === "combined")) {
    arrival = core;
  }
